...
mod tests {
    ...
    mod windows_tests {
        ...
        impl ExtendedTestContext {
            ...
            pub fn make_inaccessible_dir(&mut self, name: &str) -> Result<PathBuf, TestSetupError> {
                let dir_path = self.vanilla.temp_dir.path().join(name);

                fs::create_dir(&dir_path)?;
                self.strip_permissions(&dir_path)?;

                // Track for cleanup
                self.stripped_dirs.push(dir_path.clone());

                Ok(dir_path)
            }

            pub fn make_inaccessable_file(&mut self, path: &Path) -> Result<(), TestSetupError> {
                fs::write(path, b"test")?;
                self.strip_permissions(&path)?;
                
                // Track for cleanup
                self.stripped_files.push(path.to_path_buf());

                Ok(())
            }

            fn get_icacls_path(&self) -> Result<PathBuf, TestSetupError> {
                let system_root = env::var("SystemRoot").map_err(|e| TestSetupError::SystemRootVariableNotFound(e))?;
                let icacls_path = Path::new(&system_root).join("system32").join("icacls.exe");

                if !icacls_path.exists() {
                    return Err(TestSetupError::IcaclsNotFound());
                }

                Ok(icacls_path)
            }

            fn strip_permissions(&self, path: &Path) -> Result<(), TestSetupError> {
                let icacls_path = self.get_icacls_path()?;

                let output = Command::new(&icacls_path)
                    .args(&[
                        path.to_str().ok_or_else(|| TestSetupError::InvalidPath(path.to_string_lossy().to_string()))?,
                ...
                if !output.status.success() {
                    ...
                    return Err(TestSetupError::IcaclsCommandError(stderr.to_string()))
                }

                Ok(())
            }

            fn restore_permissions(&self, path: &Path) -> Result<(), TestSetupError> {
                let icacls_path = self.get_icacls_path()?;
        ...
        impl Drop for ExtendedTestContext {
            fn drop(&mut self) {
                
                // Restore files first
                for file in &self.stripped_files {
                    if let Err(err) = self.restore_permissions(file) {
                        eprintln!("Warning: Failed to restore permissions for file {:?}: {}", file, err);
                    }
                }
                
                // Then restore directories (deepest first)
                self.stripped_dirs.sort_by_key(|p| std::cmp::Reverse(p.components().count()));

                for dir in &self.stripped_dirs {
                    if let Err(err) = self.restore_permissions(dir) {
                        // Log but don't panic in Drop
                        eprintln!("Warning: Failed to restore permissions for {:?}: {}", dir, err);
                        ...
                        // Try icacls again
                        let _ = self.restore_permissions(dir);
                    }
                }
            }
        }
